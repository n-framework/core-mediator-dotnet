    ...
    where TRequest : IMessage
{
    ...
    protected override async ValueTask RemoveFromCacheAsync(string pattern, CancellationToken cancellationToken)
    {
        ...
        byte[]? groupCache = await cache.GetAsync(pattern, cancellationToken).ConfigureAwait(false);
            ...
            try
            {
                ...
                if (keys is not null)
                {
                    foreach (string key in keys)
                    {
                        ...
                        LogRemovedKeyFromGroup(logger, key, pattern, null);
                    }
                }
            }
            catch (JsonException ex)
            {
                LogCorruptedGroupKey(logger, pattern, ex);
                await cache.RemoveAsync(pattern, cancellationToken).ConfigureAwait(false);
                return;
            }
        }
